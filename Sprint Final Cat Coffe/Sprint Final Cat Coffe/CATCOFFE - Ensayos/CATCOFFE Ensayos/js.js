...
// 👉 Vaciar carrito automáticamente al abrir el menú
let carrito = [];
...
function renderCarrito() {
  ...
  listaCarrito.parentElement.style.display = carrito.length > 0 ? "block" : "none";
...
// Mostrar total general dinámicamente
let totalDiv = document.getElementById("total-general");
if (!totalDiv) {
  totalDiv = document.createElement("div");
  totalDiv.id = "total-general";
  
  // Estilos tipo "cuadrito"
  totalDiv.style.marginTop = "10px";
  totalDiv.style.fontWeight = "bold";
  totalDiv.style.padding = "5px 10px";
  totalDiv.style.border = "2px solid #000"; // borde del cuadrito
  totalDiv.style.display = "inline-block"; // para que no ocupe todo el ancho
  totalDiv.style.borderRadius = "5px"; // esquinas redondeadas
  totalDiv.style.backgroundColor = "#f0f0f0"; // color de fondo
  listaCarrito.parentNode.appendChild(totalDiv);
}
totalDiv.textContent = carrito.length > 0 ? `Total: $${totalGeneral}` : "";
}
...
// 👉 Mostrar carrito solo al hacer scroll si tiene productos
window.addEventListener("scroll", () => {
  if (carrito.length === 0) return;

  const contenedor = listaCarrito.parentElement;
  contenedor.style.display = window.scrollY > 100 ? "block" : "none"; // ajusta 100px a tu gusto
});
